...
public class Laporan1Activity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        tableView = findViewById(R.id.table_laporan1);
    ...
    private void getLaporan1(String tanggal){
        StringRequest stringRequest = new StringRequest(GET, RentalApi.LAPORAN1 + tanggal, new Response.Listener<String>() {
            @Override
            public void onResponse(String response) {
                Gson gson = new Gson();
                Laporan1Response laporan1Response = gson.fromJson(response, Laporan1Response.class);
                data = laporan1Response.getData();
                Toast.makeText(Laporan1Activity.this, laporan1Response.getMessage(), Toast.LENGTH_SHORT);
            }
            ...
            public void onErrorResponse(VolleyError error) {
                try{
                    ...
                    Toast.makeText(Laporan1Activity.this,
                            errors.getString("message"), Toast.LENGTH_SHORT).show();
                }catch (Exception e){
                    Toast.makeText(Laporan1Activity.this, e.getMessage(),
    ...
    private boolean validateForm(String bulan, String tahun){
        if (bulan.isEmpty() || bulan.isEmpty()){
            Toast.makeText(Laporan1Activity.this, "Tahun atau Bulan Kosong", Toast.LENGTH_SHORT).show();
            return false;
        } else if( Integer.valueOf(bulan) > 12 || Integer.valueOf(bulan) < 1){
            Toast.makeText(Laporan1Activity.this, "Bulan harus di antara 1 - 12", Toast.LENGTH_SHORT).show();
            return false;
        }
        return true;
    }
    public void setTableAdapter(){
        String[] header = {"Tipe","Nama","Jumlah","Harga"};
        int x = data.size();
        String[][] body = new String[x][4];
        for(int i = 0; i < x; i++){
            body[i][0] = data.get(i).getTipeAset();
