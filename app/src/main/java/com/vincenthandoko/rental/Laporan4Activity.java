...
public class Laporan4Activity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        tableView = findViewById(R.id.table_laporan4);
    ...
    private void getLaporan4(String tanggal){
        StringRequest stringRequest = new StringRequest(GET, RentalApi.LAPORAN4 + tanggal, new Response.Listener<String>() {
            @Override
            public void onResponse(String response) {
                Gson gson = new Gson();
                Laporan4Response laporan4Response = gson.fromJson(response, Laporan4Response.class);
                data = laporan4Response.getData();
                Toast.makeText(Laporan4Activity.this, laporan4Response.getMessage(), Toast.LENGTH_SHORT);
            }
            ...
            public void onErrorResponse(VolleyError error) {
                try{
                    ...
                    Toast.makeText(Laporan4Activity.this,
                            errors.getString("message"), Toast.LENGTH_SHORT).show();
                }catch (Exception e){
                    Toast.makeText(Laporan4Activity.this, e.getMessage(),
    ...
    private boolean validateForm(String bulan, String tahun){
        if (bulan.isEmpty() || bulan.isEmpty()){
            Toast.makeText(Laporan4Activity.this, "Tahun atau Bulan Kosong", Toast.LENGTH_SHORT).show();
            return false;
        } else if( Integer.valueOf(bulan) > 12 || Integer.valueOf(bulan) < 1){
            Toast.makeText(Laporan4Activity.this, "Bulan harus di antara 1 - 12", Toast.LENGTH_SHORT).show();
            return false;
        }
        return true;
    }
    public void setTableAdapter(){
        String[] header = {"ID","Nama","Rata-Rata","Jumlah Transaksi"};
        int x = data.size();
        String[][] body = new String[x][4];
        for(int i = 0; i < x; i++){
            body[i][0] = data.get(i).getId();
            body[i][1] = data.get(i).getNamaDriver();
            body[i][2] = String.valueOf(data.get(i).getRataRata());
            body[i][3] = String.valueOf(data.get(i).getJumlahTransaksi());
        }
